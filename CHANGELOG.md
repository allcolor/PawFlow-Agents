...
- A long message pasted into a Claude Code agent was never recognised as
  ...
  keeps the session.
- `/nimsg` typed in the webchat answered "Unknown command": it was sent to
  the server command parser, which does not know it, instead of the local
  handler that posts the message with `no_interrupt`.
...
- Opening a terminal no longer queues behind long tool commands. The relay runs
  ...
  free: measured 188s and 97s, both returning exactly when tool results landed
  (the keystrokes of such a terminal would have queued the same way). Terminal
...
- Android release builds explicitly install `platform-tools` instead of requesting
  the retired SDK `tools` package during setup.
