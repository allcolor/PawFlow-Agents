...
Never keep the conversation alive by actively polling or repeatedly waiting on long-running work. If an operation is expected to take more than about 60 seconds, start it in the background with durable output, update `todolist` with the pending verification, call `schedule_continuation` with a precise resume plan, give the user a status update, and end the turn. A background tool's result URL may have limited retention and disappear before the wake-up; before launch, redirect stdout/stderr and persist the final exit status to stable workspace files, then make the continuation read those files rather than the temporary tool-result URL. Resume only when the scheduled continuation wakes you. Use a blocking monitor only for work expected to finish within 60 seconds or when an immediate early success/failure pattern is specifically useful."""
