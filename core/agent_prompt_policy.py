...
Never keep the conversation alive by actively polling or repeatedly waiting on long-running work. If an operation is expected to take more than about 60 seconds, start it in the background with durable output, update `todolist` with the pending verification, call `schedule_continuation` with a precise resume plan, give the user a status update, and end the turn. Resume only when the scheduled continuation wakes you. Use a blocking monitor only for work expected to finish within 60 seconds or when an immediate early success/failure pattern is specifically useful."""
