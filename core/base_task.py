...
Provides common functionality and standard structure.
"""
...
class BaseTask(VariableResolverMixin, Task, ABC):
    ...
    Manages validation, variable resolution and utility functions.
    """
    ...
    def get_attribute(self, flowfile: FlowFile, key: str, default: Optional[str] = None) -> Optional[str]:
        ...
        # Inherit attributes from parent if specified
        if parent_flowfile:
    ...
    def read_content(self, flowfile: FlowFile) -> bytes:
        """
        Read the content of a FlowFile.
    ...
    def split_content(self, content: bytes, split_by: bytes = b"\n") -> List[bytes]:
        ...
        Args:
            ...
            split_by: Separator character (bytes)
            
        Returns:
            List of split contents
        """
        if isinstance(content, str):
            content = content.encode('utf-8')
        
        parts = content.split(split_by)
        return [p for p in parts if p]  # Filter empty parts
    
    def merge_content(self, contents: List[bytes], separator: bytes = b"\n") -> bytes:
        """
        Merge multiple contents.
        
        Args:
            contents: List of contents
            separator: Separator between contents (bytes)
    ...
    def parse_json(self, content: str) -> Dict[str, Any]:
        ...
        Raises:
            TaskError: If JSON is invalid
        """
        try:
            return json.loads(content)
        except json.JSONDecodeError as e:
            raise TaskError(f"Invalid JSON: {e}")
    ...
    def serialize_json(self, data: Any) -> str:
        ...
        try:
            ...
            raise TaskError(f"JSON serialization error: {e}")
    ...
    def get_task_id(self) -> str:
        ...
        Returns:
            Task ID (based on class name)
        """
        # This method will be filled in by the parent flow
