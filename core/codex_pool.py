"""Codex CLI container pool manager.

One container per Codex exec (1:1). The pool keeps a small set of
...
class CodexPool:
    ...
    Usage:
        ...
        proc = pool.exec_codex(container, session_dir, codex_args)
        # ... stream proc.stdout ...
        pool.release(container)            # docker rm -f (not returned to pool)
    """

    _instance: Optional['CodexPool'] = None
    _instance_lock = threading.Lock()

    @classmethod
    def instance(cls) -> 'CodexPool':
        ...
        with self._lock:
            ...
            if not workspace_mount_args and self._ready:
                ...
                if len(self._active) + len(self._ready) >= self.max_containers:
                    raise RuntimeError(
                        f"Codex pool exhausted: active={len(self._active)} "
        ...
        if spawn_needed:
            ...
            with self._lock:
                ...
                if (len(self._active) + len(self._ready)
                    ...
                    raise RuntimeError(
                        f"Codex pool exhausted during race: "
    ...
    def exec_codex(self, container_name: str, session_dir: str,
                    codex_args: list, extra_env: dict = None,
                    **popen_kwargs) -> subprocess.Popen:
        """Start a codex process inside a pool container.

        Args:
            container_name: container from acquire()
            session_dir: path INSIDE the container that becomes CODEX_HOME
        ...
        # before exec'ing claude — Codex refuses to run as root.
        #
        ...
        exec_args = [
            ...
            "-e", "USER=pawflow",
            # CODEX_HOME redirects ~/.codex/ to the per-conv slot — codex
            # writes config.toml + sessions/YYYY/MM/DD/rollout-*.jsonl + auth.json
            # under here, so each conv has isolated session history.
            "-e", f"CODEX_HOME={_ns_workdir}/.codex",
        ...
        # __PF_CODEX_PID= value (mirrored from shared self._stderr_buffer,
        ...
        import shlex
        _codex_quoted = " ".join(shlex.quote(str(a)) for a in codex_args)
        _shell_script = (
    ...
    def _spawn_container(self, workspace_mount_args: Optional[List[str]] = None) -> str:
        ...
        """
        import uuid
        name = f"pf-codex-pool-{uuid.uuid4().hex[:8]}"
        workspace_mount_args = list(workspace_mount_args or [])
