"""Gemini CLI container pool manager.

One container per Gemini exec (1:1). The pool keeps a small set of
...
class GeminiPool:
    ...
    Usage:
        ...
        proc = pool.exec_gemini(container, session_dir, gemini_args)
        # ... stream proc.stdout ...
    ...
    def _register_death_handlers(self):
        ...
        def _kill_all(*_args, **_kwargs):
            ...
            # Kill live gemini containers FIRST so the registry can release
    ...
    def __init__(self):
        ...
        self.image = os.environ.get(
            "PAWFLOW_GEMINI_IMAGE", "pawflow-claude-code:latest")
        self.max_containers = int(os.environ.get(
            "PAWFLOW_GEMINI_POOL_MAX", "50"))
        ...
        with self._lock:
            ...
            if not workspace_mount_args and self._ready:
                ...
                if len(self._active) + len(self._ready) >= self.max_containers:
                    raise RuntimeError(
                        f"Gemini pool exhausted: active={len(self._active)} "
        ...
        if spawn_needed:
            ...
            with self._lock:
                ...
                if (len(self._active) + len(self._ready)
                    ...
                    raise RuntimeError(
                        f"Gemini pool exhausted during race: "
    ...
    def exec_gemini(self, container_name: str, session_dir: str,
                    gemini_args: list, extra_env: dict = None,
                    **popen_kwargs) -> subprocess.Popen:
        """Start a gemini process inside a pool container.

        Args:
            container_name: container from acquire()
            session_dir: path INSIDE the container that becomes the per-conv
                         home (e.g. /cc_sessions/<user>/<conv>/<agent>) —
                         GEMINI_CLI_HOME points to <session_dir>/.gemini so
        ...
        # before exec'ing claude — Gemini refuses to run as root.
        #
        ...
        exec_args = [
            ...
            # HOME = the per-conv slot — gemini writes ~/.gemini/settings.json,
            # ~/.gemini/oauth_creds.json and ~/.gemini/tmp/<hash>/chats/ under HOME,
            # so all per-conv state is isolated to /cc_sessions/<conv>/<agent>/.gemini.
        ...
        #   printf "__PF_GEMINI_PID=$$" 1>&2                 (bash PID = claude PID
        #                                                     after chained exec)
        ...
        # __PF_GEMINI_PID= value (mirrored from shared self._stderr_buffer,
        ...
        import shlex
        _gemini_quoted = " ".join(shlex.quote(str(a)) for a in gemini_args)
        _shell_script = (
    ...
    def _spawn_container(self, workspace_mount_args: Optional[List[str]] = None) -> str:
        ...
        """
        import uuid
        name = f"pf-gemini-pool-{uuid.uuid4().hex[:8]}"
        workspace_mount_args = list(workspace_mount_args or [])
        ...
        run_args = [
            ...
            "--name", name,
            "--init",
            *pawflow_container_labels("gemini-pool"),
            "--cpus", self.cpu_limit,
