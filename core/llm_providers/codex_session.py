...
class CodexSessionMixin:
    ...
    def _codex_setup_mcp_config(self, workdir: str, user_id: str = "",
        ...
        # Build env as a TOML inline table on the same line as the MCP
        # server definition. Both `[mcp_servers.X.env]` sub-tables and
        # `env = { K = "V" }` inline tables are valid TOML, but the
        # inline form keeps the env scoped to ONE block and avoids any
        # ambiguity in codex's MCP config parser — the sub-table form
        # was failing to expose vars to the bridge subprocess at runtime
        # (PAWFLOW_INTERNAL_TOKEN missing → cookie-less WS upgrade → 403
        # on /ws/tools/_tool_relay → cascade of auth failures).
        env_pairs = {
        ...
        env_inline = ", ".join(
            f'{k} = "{_toml_escape(v)}"' for k, v in env_pairs.items())
        toml = (
