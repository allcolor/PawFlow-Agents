...
Supports .pfp (PawFlow Plugin) archives — zip files containing:
...
class PluginDescriptor:
    ...
    def from_dict(cls, data: dict) -> 'PluginDescriptor':
        return cls(
            ...
            min_pawflow_version=data.get("min_pawflow_version", "1.0.0"),
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "min_pawflow_version": self.min_pawflow_version,
            "tasks": self.tasks,
...
class PluginManager:
    ...
    def _check_compatibility(self, descriptor: PluginDescriptor):
        """Check if plugin is compatible with current PawFlow version."""
        # Simple version check (major.minor)
        required = descriptor.min_pawflow_version.split('.')
        current = __version__.split('.')
        try:
            if int(current[0]) < int(required[0]):
                raise ValueError(
                    f"Plugin {descriptor.id} requires PawFlow >= {descriptor.min_pawflow_version}, "
