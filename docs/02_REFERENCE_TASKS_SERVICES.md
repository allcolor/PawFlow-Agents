...
- Conversation view with streaming assistant output, tool calls, tool results, approvals, background tools, and active-agent controls. The View menu stores `chat.view_mode` at conversation scope: `classic` (the default) retains the existing transcript, while `simplified` renders each correlated user turn as the user message, one expandable live activity block with Messages, Thinking, Tool calls, and Artifacts tabs, then the same canonical final assistant message. Durable `turn_id` and `turn_final` metadata make reload, pagination, and reconnect reconstruction deterministic. Reconstruction never depends on that metadata being present: when no row of a turn carries `turn_final`, the display classifier marks the last visible assistant row of the turn instead and flags it `turn_final_derived`, so conversations recorded before the feature — and turns whose terminal patch never landed — still render their answer outside the activity block. A turn that produced no visible answer is left without a final row; the view never manufactures one. Successful `show_file` results appear once in Artifacts; actionable approvals, questions, errors, and notifications remain top-level. In classic mode, `chat.group_technical_messages` can collapse consecutive technical rows when it resolves to a truthy value (`true`, `1`, `yes`, `on`); the default is `true`. Tool-call groups keep a stable `tc_id` boundary so a reload does not merge unrelated tools into one technical details block. View options write conversation-scoped parameters and reload through the canonical history path. The floating scroll controls use explicit top/bottom navigation; the top button does not trigger history lazy-loading by itself.
