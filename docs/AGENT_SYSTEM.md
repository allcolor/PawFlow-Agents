...
The raw tail comes from the transcript, personalized for the agent being compacted (`load_transcript_tail_for_agent`). Delegate rows (`source.type == "agent_delegate"`) get the same per-agent copy that `_route_delegate_message` wrote live: the sender keeps its row as `[delegate <from> → <to>]:`, the receiver gets it as a user turn (`Here is agent '<from>''s reply to your delegate:` / `Here is a message from agent '<from>':`), other agents see requests only as `[<from> to agent <to>]:`, and `self_only` reply internals stay with the responder. A rebuilt context never replays another agent's reply as the receiver's own assistant turn.

The sender's copy of a delegate-mode turn is stored as canonical rows, like any own-context write: the anchor plus its `thinking` and `tool_call` child rows (`parent_message_id` = anchor). A delegate-mode turn that called tools therefore reloads with its tool calls, and its results are not dropped as orphans. The receiver and the shared broadcast get the anchor only, without reasoning or tool calls. The transcript tail keeps the same children for their sender.
...
`tasks.ai.context_usage.compute_context_usage(conversation_id, agent_name, user_id=...)` is the single server-side calculation point for the gauge. It resolves the agent's LLM service, computes the effective context window, and returns `used / max / pct`.

Direct API providers count the full PawFlow provider context: the messages sent in the request plus the provider system prompt and tool definitions (`core/token_counter.count_context_tokens`).

Stateful CLI providers are measured, not reconstructed. The window belongs to the CLI session and holds what PawFlow cannot enumerate — the provider's own system prompt, its tool schemas, and whatever the agent chose to read. Each of the six CLI providers reports its prompt size after every exchange through `record_observed_cli_context` / `record_observed_wire_usage` (`core/llm_providers/cli_shared.py`), and that measurement is the gauge. PawFlow's own reconstruction of the stored messages is the fallback used until the first measurement lands — it is never `0` any more, so a provider that goes quiet cannot strand the gauge at an empty window. Providers that also report their own window (Codex) supply the denominator through `record_observed_cli_window`, in preference to the configured `max_context_size`.

The stored PawFlow conversation is deliberately *not* what the gauge reports for a CLI provider: it is never sent anywhere in one piece, so compacting against its size would destroy history to protect a window that does not exist. What overflows is the CLI's own context, and that is what is measured.

The injected `Context: ~x/y` note derives from the same number, but it must read the gauge rather than recount what is being sent. On a warm CLI session `_alc_with_provider_system_prompt` drops the system prompt and the history the CLI already holds, so the outgoing provider context is only the turn's delta — counting it announced `~864/800000` to an agent whose session the gauge measured at `77965`, and `~602307` for the same conversation one restart earlier while cold. `_alc_inject_dynamic_metadata` therefore reads `compute_context_usage` on CLI providers, falling back to counting the provider context only when no measurement exists yet (cold start) — exactly the case where the provider context *is* the whole context. The API path still counts the provider context directly. The note's denominator stays local: `st._max_ctx` also budgets compaction and must not move with it.
