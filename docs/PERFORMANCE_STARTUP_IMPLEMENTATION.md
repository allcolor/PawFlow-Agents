...
- `tests/test_distribution_assets.py`: builds source and wheel archives with fresh and stale manifests, then checks asset/license bytes and generated-cache exclusions. These builds use `--no-isolation`, so the test environment must contain `build`, `setuptools>=68.0` and `wheel`; both the development extra and branch CI install them explicitly.
