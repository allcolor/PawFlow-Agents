...
| Check whether your delegates are still running | `delegate_status` | Transcript-backed shared/isolated/flash state merged with live runtime details; survives context compaction |
| Retrieve a finished delegate's output you never received | `delegate_result` | Pull by task_id from the runtime cache or durable reply/trace; full retained response text |
