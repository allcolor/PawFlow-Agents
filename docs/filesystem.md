...
A retried request keeps its `request_id`, and the relay runs a request id at most once (`pawflow_relay/command_ledger.py`). Its ledger outlives connections: a retry that arrives while the first run is still going is not run again, and gets that run's result; a retry that arrives after it ended gets the recorded result (kept for five minutes, at most 256 results and 64 MiB). Results and streamed output are always sent on the relay's current connection, not on the one the command arrived on, so a run that outlives a reconnect is not lost on the dead socket. Before this, a watchdog reconnect during a long `exec` (2026-09-26) ran it a second time in parallel and lost the first run's result. When the relay watchdog forces a reconnect it now logs whether the send lock was held and the last send, so the cause of the silence is on record.
...
When a Docker relay is started with `--allow-local`, `local=true` operations are forwarded to the host helper. The Docker worker performs an authenticated helper health check before registering and every 15 seconds afterward. A transient helper failure leaves the main relay WebSocket connected while later requests retry the dynamic host route. On Windows, an aborted client can make `accept()` report `ConnectionAbortedError` or `ConnectionResetError`; the host helper treats those failures as transient and continues accepting connections instead of silently terminating. Relay Desktop starts a tracked bridge process directly in WSL and waits for its listener before launching the Docker worker. The WSL listener uses a different port from the Windows helper target, preventing loopback route discovery from reconnecting the bridge to itself. The worker reaches that listener through Docker's `host-gateway`; the bridge selects an authenticated route to Windows using WSL loopback or its NAT gateway. This avoids stale Wi-Fi/VPN addresses, avoids Docker Desktop host-network semantics, and works in both WSL mirrored and NAT modes. Closing Relay Desktop's bridge pipe stops the WSL process. Every helper request is authenticated. For Windows hosts whose project root is a UNC path such as `\\wsl$\...`, cmd-based execution uses `pushd`/`popd` instead of setting the process current directory to the UNC path, because `cmd.exe` cannot run with a UNC cwd.
