...
PyFi2 provides a unified filesystem abstraction that allows flows and agents to access files from various sources — the user's local machine, cloud storage (Google Drive, OneDrive), or the browser — through a consistent interface with enforced permissions.

**Core principle:** The PyFi2 server does NOT access its own filesystem by default. All file access goes through an explicitly configured filesystem service.
...
|------|-------------|-------------|----------|
| `localFilesystem` | HTTP relay to user's machine | Yes | `pyfi2_fs_relay.py` script |
| `wsFilesystem` | WebSocket relay (persistent connection) | Yes | `pyfi2_fs_relay_ws.py` script |
...
python tools/pyfi2_fs_relay.py --port 9876 --dir /home/user/project --secret mysecret

# Read-only access
python tools/pyfi2_fs_relay.py --port 9876 --dir /data --secret mysecret --readonly

# Bind to all interfaces (for remote access)
python tools/pyfi2_fs_relay.py --port 9876 --dir /data --secret mysecret --bind 0.0.0.0
```

### 2. Install the service in PyFi2
...
```bash
python tools/pyfi2_fs_relay_ws.py --port 9877 --dir /home/user/project --secret mysecret
```
...
```
/service install serverFilesystem staging root=/var/pyfi2/staging,mode=readwrite
```
