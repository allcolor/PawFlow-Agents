...
Chat Completions cache usage is normalized before it reaches usage tracking. OpenAI's `prompt_tokens_details.cached_tokens` is subtracted from `prompt_tokens`; DeepSeek's top-level `prompt_cache_hit_tokens` and `prompt_cache_miss_tokens` take precedence when present. PawFlow records misses as `tokens_in` and hits as `cache_read_tokens`, while `prompt_tokens` remains the provider's total prompt count rather than a third billable bucket. The same mapping is applied to non-streaming responses and to the final usage-bearing streaming chunk, so the usage ledger, cost calculation, conversation source metadata, and live telemetry all receive the native cache split without double counting.
