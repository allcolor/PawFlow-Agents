...
| Type | Purpose |
|---|---|
| `llmConnection` | LLM service configuration for direct API providers, outbound ACP agents, and CLI-backed providers (`openai`, `openai-responses`, `anthropic`, `acp`, `claude-code-interactive`, `antigravity-interactive`, `codex-interactive`, `gemini`). The `claude-code` (`cc -p`) and `codex-app-server` agent transports are legacy and remain available for existing configurations. |
