...
| Type | Purpose |
|---|---|
| `llmConnection` | LLM service configuration for direct API providers and CLI-backed providers (`openai`, `openai-responses`, `anthropic`, `claude-code`, `claude-code-interactive`, `antigravity-interactive`, `codex-app-server`, `codex-interactive`, `gemini`). |
...
| `llmCredentialOAuthProvider` | Encrypted OAuth credential pool for CLI-backed LLM providers. Canonical providers are `claude-code`, `codex-app-server`, and `gemini`; `claude-code-interactive` reuses `claude-code`, `codex-interactive` reuses `codex-app-server`, and `antigravity-interactive` reuses `gemini`. |
