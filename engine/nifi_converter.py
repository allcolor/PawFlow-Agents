"""NiFi → PyFi2 flow converter.

Parses NiFi flow exports (XML templates or JSON REST API format)
and converts them to PyFi2 flow dicts.

Handles:
- Processor mapping (NiFi processor type → PyFi2 task type)
...
# ============================================================================
# NiFi processor → PyFi2 task mapping
# ============================================================================
...
# NiFi controller service → PyFi2 service mapping
...
# NiFi relationship → PyFi2 relation type
...
class ConversionResult:
    """Result of NiFi → PyFi2 conversion."""
    flow: Dict[str, Any]
...
class NiFiConverter:
    """Converts NiFi flow exports to PyFi2 flow dicts."""

    def convert_xml(self, xml_content: str) -> ConversionResult:
        """Convert NiFi XML template/flow to PyFi2 flow dict."""
        try:
    ...
    def convert_json(self, json_content: str) -> ConversionResult:
        """Convert NiFi JSON (REST API format) to PyFi2 flow dict."""
        try:
    ...
    def _convert_process_group(self, pg_element, source_format="xml") -> ConversionResult:
        """Convert a NiFi processGroup XML element to PyFi2 flow dict.
        ...
        id_map = {}  # NiFi UUID → PyFi2 task_id
    ...
    def _convert_xml_processor(self, proc_element) -> Tuple[Optional[str], Dict[str, Any], List[ConversionWarning]]:
        """Convert a single NiFi processor XML element to PyFi2 task config."""
        warnings = []
        ...
        if not pyfi_type or pyfi_type == "_unmapped":
            ...
            warnings.append(ConversionWarning(
                ...
                f"No PyFi2 equivalent for '{nifi_type}'. Replaced with log task.",
            ))
    ...
    def _convert_json_flow(self, flow_data: Dict[str, Any], pg_data: Optional[Dict[str, Any]] = None) -> ConversionResult:
        """Convert NiFi JSON REST API flow to PyFi2 flow dict."""
        warnings = []
        unmapped = []
        script_procs = []
    ...
    def _map_processor_type(self, nifi_type: str) -> str:
        """Map NiFi processor class to PyFi2 task type."""
    ...
    def _map_relationship(self, relationships: List[str]) -> str:
        """Map NiFi relationship names to PyFi2 relation type."""
    ...
    def _map_properties(self, pyfi_type: str, nifi_props: Dict[str, str]) -> Dict[str, Any]:
        """Map NiFi processor properties to PyFi2 task parameters."""
        params = {}
