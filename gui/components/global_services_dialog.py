...
def global_services_dialog():
    ...
    # --- Install new service (hide when editing to reduce clutter) ---
    if not editing:
        st.markdown("---")
            ...
            if new_type:
                ...
                if schema:
                    ...
                    new_config = render_schema_fields(
                        schema, {}, key_prefix="gsvc_new_cfg"
                    )
                else:
...
def _render_config_editor(registry, sdef):
    ...
    with st.form(key=f"gsvc_form_{sdef.service_id}"):
        # Rename
        ...
        if schema:
            ...
            edited = render_schema_fields(
                schema, sdef.config,
                key_prefix=f"gsvc_edit_{sdef.service_id}"
            )
        else:
            edited = {}
        ...
        with save_cols[0]:
            submitted = st.form_submit_button(
                f"💾 {t('common.save')}", type="primary"
            )
        with save_cols[1]:
            cancelled = st.form_submit_button(t("common.cancel"))
