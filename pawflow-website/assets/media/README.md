...
| P0 | `diagrams/published-mcp-read-only.webp` | 1600x900 | How-to: published MCP client | Evidence-based architecture diagram showing web connector and local stdio entry paths, the agent-scoped published endpoint, allowlist plus read-only discovery and execution gates, optional read-only workspace relay, and audited MCP results. |
...
| P0 | `video/chatgpt-mcp-live-workspace.mp4` | 40.5s | Homepage + how-to: published MCP client | Real ChatGPT session reading current Git history from MyWorkspace through PawFlow MCP in read-only mode. Poster is `video/chatgpt-mcp-live-workspace-poster.webp`. |
| P1 | `video/private-gateway.mp4` | 13.6s | Homepage: Private Gateway | Real Matrix gateway skin with masked access-code entry before PawFlow sign-in. Poster is `video/private-gateway-poster.webp`. |
| P1 | `video/android-app-native.mp4` | 18.4s | How-to: Android app | Real APK recording: saved-server selection, connection, and native tabbed webchat. The private server URL is blurred throughout the selector sequence. Poster is `video/android-app-native-poster.webp`. |
...
| P1 | `video/comfyui-windows-relay.mp4` | 18.5s | How-to: ComfyUI | Mobile PawFlow conversation submits a reviewed ComfyUI workflow through a Windows relay, waits for completion, validates the artifact, and returns the result. Poster is `video/comfyui-windows-relay-poster.webp`. |
