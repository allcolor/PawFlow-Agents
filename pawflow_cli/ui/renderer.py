...
# Color/verb/diff/summary helpers live in _render_helpers to keep this module
# <=800 lines; re-exported so pawflow_cli.ui.renderer stays the public path
# (app.py imports _random_verb from here).
from pawflow_cli.ui._render_helpers import (  # noqa: F401
    _TOOL_DISPLAY_NAMES,
    _agent_color,
    _lang_from_path,
    _random_verb,
...
class TerminalRenderer:
    ...
    def _strip_tool_wrapper(self, text: str) -> str:
        ...
        if text.startswith("<tool_output tool="):
            ...
            if nl >= 0:
                text = text[nl + 1:]
            close_idx = text.rfind("</tool_output>")
            if close_idx >= 0:
