    ...
    try:
        if args.command == "server":
            if args.server_command == "add":
                _print_server(add_server(args.name, args.url, args.gateway_key))
                return 0
            if args.server_command == "login":
                _print_server(_login_server(args.name))
                return 0
            if args.server_command == "list":
                for profile in list_servers():
                    _print_server(profile)
                return 0

        if args.command == "workspace":
            if args.workspace_command == "add":
                _print_workspace(add_workspace(
                    args.name,
                    args.server,
                    args.path,
                    mode=args.mode,
            ...
            if args.workspace_command == "list":
                for share in list_workspaces():
                    _print_workspace(share)
                return 0
        ...
        if args.command == "status":
            ...
            print(f"servers={len(servers)} workspaces={len(workspaces)}")
            return 0
