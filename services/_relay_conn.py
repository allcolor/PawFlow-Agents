...
class _RelayConnMixin:
    ...
    async def _dispatch_relay_msg(self, msg, writer, service, send_lock, relay_tasks):
        ...
        if mtype in ('result', 'error'):
            ...
            try:
                from services.vnc_proxy import dispatch_vnc_ws_data
                dispatch_vnc_ws_data(service._service_id,
                                     msg.get('session_id', ''),
                                     msg.get('data', ''),
                                     msg.get('opcode', 2))
            except Exception as e:
                logger.debug('desktop_ws_data dispatch failed: %s', e,
                             exc_info=True)
        elif mtype == 'desktop_ws_close':
            try:
                from services.vnc_proxy import dispatch_vnc_ws_close
                dispatch_vnc_ws_close(service._service_id,
                                      msg.get('session_id', ''))
            except Exception as e:
