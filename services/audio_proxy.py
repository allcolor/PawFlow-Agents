...
def audio_ws_proxy(client_sock, path_params: dict, meta: dict):
    ...
    def _backend_to_browser():
        """Send queued packets to browser WS at a fixed 50 pkts/s rate.

        Rate smoother: reader fills the queue at PulseAudio's rate (jittery),
        writer drains exactly 1 packet every 20ms (stable). If queue is empty,
        skip (no silence injection — browser handles gaps). If queue grows
        beyond 100 packets (~2s), drop oldest to prevent unbounded lag.
        """
        ...
        try:
            while not stop.is_set():
                ...
                _next_tick += _TICK
                # Prevent drift accumulation: if we fell behind, reset
                if _next_tick < time.monotonic() - 0.1:
                    _next_tick = time.monotonic() + _TICK
                # Drop excess if queue too large (prevent lag buildup)
