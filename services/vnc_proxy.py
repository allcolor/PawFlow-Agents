...
def vnc_ws_proxy(client_sock, path_params: dict, meta: dict):
    ...
    t1 = threading.Thread(target=_relay, args=(client_sock, backend_sock, "browser→docker"),
                          daemon=True)
    t2 = threading.Thread(target=_relay, args=(backend_sock, client_sock, "docker→browser"),
                          daemon=True)
    t1.start()
    t2.start()
