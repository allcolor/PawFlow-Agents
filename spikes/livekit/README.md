...
- 2026-07-17 (tier-3 bench, Gemini Live video — `bench/driver3.py`, `BENCH_PROVIDER=gemini`): BLOCKED ON BILLING, not on code — the full chain works (worker dispatch, bootstrap with the Google key, `livekit-plugins-google` connects to the Gemini Live endpoint) but Google closes the session with `1011 Your prepayment credits are depleted` (recharge at ai.studio/projects). The driver publishes a synthetic red-square camera track + a spoken color question and asserts the agent names the color; rerun as-is once the project has credits.
