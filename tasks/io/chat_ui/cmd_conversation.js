...
function cmdSearch(text, parts, cmd) {
  ...
  action$('load_history', { conversation_id: conversationId, limit: 500, offset: 0 })
    .subscribe(data => {
      ...
      if (found.length) addMsg('system', 'Found ' + found.length + ' match(es):\n' + found.slice(0, 20).join('\n'));
      else addMsg('system', 'No matches found.');
    });
  return true;
}
...
function cmdClearStore(text, parts) {
  ...
  action$('clear_store', params).subscribe(data => {
    if (data.error) addMsg('error', data.error);
    else if (data.deleted !== undefined) addMsg('system', 'FileStore: deleted ' + data.deleted + ' file(s)' + (data.scope ? ' (' + data.scope + ')' : ''));
  });
  return true;
}
...
function cmdDiff(text, parts) {
  ...
  action$('fs_exec', { service: '', command: 'git diff ' + ref, timeout: 15 })
    .subscribe(data => {
      ...
      if (!output) { addMsg('system', 'No changes.'); return; }
...
function cmdWatch() { addMsg('system', '/watch is not available in the web UI. Use the CLI for file watching.'); return true; }
function cmdClearFiles() { pendingFiles = []; addMsg('system', 'Pending attachments cleared.'); return true; }
...
function cmdLoop(text, parts) {
  ...
  if (loopArg === 'stop') {
    ...
    action$('loop_stop', { key: loopKey }).subscribe(data => {
      addMsg('system', data.stopped ? 'Loop stopped: ' + loopKey : 'Loop not found: ' + loopKey);
    });
    return true;
  }
  ...
  action$('loop_start', { conversation_id: conversationId, interval_seconds: intervalSec, prompt: loopPrompt })
    .subscribe(data => {
      if (data.started) addMsg('system', 'Loop started: every ' + intervalSec + 's — ' + loopPrompt + '\nKey: ' + data.key);
      else addMsg('error', data.error || 'Failed to start loop');
    });
  return true;
}
...
function cmdSchedulesList() {
  ...
  action$('list_schedules', { conversation_id: conversationId }).subscribe(data => {
    ...
    if (scheds.length === 0) addMsg('system', 'No scheduled rechecks for this conversation.');
    else {
...
function cmdSchedulesAdd(dateStr, reason) {
  if (!conversationId) { addMsg('system', 'No active conversation'); return; }
  if (!/^\d{14}$/.test(dateStr)) { addMsg('system', 'Invalid date format. Use YYYYMMDDHHmmss'); return; }
  action$('add_schedule', { conversation_id: conversationId, at: dateStr, reason: reason || 'manual schedule' })
    .subscribe(data => {
      if (data.error) { addMsg('error', data.error); return; }
