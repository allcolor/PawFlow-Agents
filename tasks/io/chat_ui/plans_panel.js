...
async function loadPlans() {
  ...
  try {
    ...
    list.innerHTML = '<span style="color:#e94560;font-size:12px">Failed to load plans</span>';
  }
}
...
async function updatePlanStep(planId, stepIndex, status) {
  try {
    const resp = await fetch(API, {
      method: 'POST', headers: getAuthHeaders(),
      body: JSON.stringify({
    ...
    addMsg('error', 'Failed to update step: ' + e.message);
  }
}
...
async function planAction(action, planId) {
  try {
    ...
    addMsg('error', 'Plan action failed: ' + e.message);
  }
}
...
async function assignPlanTo(planId, agent, stepRange) {
  ...
  try {
    ...
    addMsg('error', 'Assign failed: ' + e.message);
  }
}
