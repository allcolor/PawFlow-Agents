...
class TestToolRegistry(unittest.TestCase):
    ...
    def test_get_tool_definitions(self):
        ...
        assert len(defs) == 41  # builtins + memory + plan + notify + ask_user + create_tool + ask_agent + flow_manager + pawflow_help + secrets + resources + show_file + browser + link_identity + remote_exec + generate_video + assign_task + complete_task + verify_task + run_tests + web_fetch
...
class TestAgentLoopTask(unittest.TestCase):
    ...
    def test_tool_registry_default(self):
        ...
        assert len(registry.list_tools()) == 41  # builtins + memory + plan + notify + ask_user + create_tool + ask_agent + flow_manager + pawflow_help + secrets + resources + show_file + browser + link_identity + remote_exec + generate_video + assign_task + complete_task + verify_task + run_tests + web_fetch
