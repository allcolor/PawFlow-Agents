...
class TestDeploymentRegistryI18n(unittest.TestCase):
    """Test that i18n keys for deployment are present."""

    def test_en_keys_present(self):
        path = Path("gui/i18n/en.json")
        data = json.loads(path.read_text(encoding="utf-8"))
        keys = [
            "runtime.deployed_flows",
            "runtime.global_flows",
            "runtime.deploy_new",
            "runtime.deploy_and_start",
            "runtime.assign_owner",
            "runtime.owner_global",
            "runtime.undeploy",
            "runtime.flow_stopped_info",
            "runtime.no_deployments",
            "runtime.instance_params",
            "runtime.template_source",
        ]
        for k in keys:
            assert k in data, f"Missing i18n key: {k}"

    def test_fr_keys_present(self):
        path = Path("gui/i18n/fr.json")
        data = json.loads(path.read_text(encoding="utf-8"))
        assert "runtime.deployed_flows" in data
        assert "runtime.deploy_new" in data

    def test_es_keys_present(self):
        path = Path("gui/i18n/es.json")
        data = json.loads(path.read_text(encoding="utf-8"))
        assert "runtime.deployed_flows" in data
        assert "runtime.deploy_new" in data

    def test_key_consistency(self):
        """All 3 locales have the same deployment keys."""
        ...
        deploy_keys = [k for k in en if k.startswith("runtime.deploy") or
                       k in ("runtime.undeploy", "runtime.no_deployments",
                              "runtime.instance_params", "runtime.template_source",
                              "runtime.owner_global", "runtime.assign_owner",
                              "runtime.flow_stopped_info", "runtime.global_flows")]
        for k in deploy_keys:
            assert k in fr, f"FR missing: {k}"
            assert k in es, f"ES missing: {k}"
