...
# ── RelayHTTPBackend + LocalFilesystemService removed ──────────────
# (old HTTP relay replaced by unified WS-based FilesystemService)
...
class TestServerFilesystemBackend(unittest.TestCase):
    ...
    def test_find_replace(self):
        result = self.backend.find_replace("hello.txt", "World", "PawFlow")
        self.assertEqual(result["replacements"], 1)
        data = self.backend.read_file("hello.txt")
        self.assertEqual(data, b"Hello PawFlow")
