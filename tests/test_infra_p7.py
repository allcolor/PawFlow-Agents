...
class TestRateLimiter:
    ...
    def test_remaining_count(self):
        from api.rate_limit import RateLimiter
        limiter = RateLimiter(max_requests=5, window_seconds=60)
        _, remaining = limiter.is_allowed("c")
        assert remaining == 4
        _, remaining = limiter.is_allowed("c")
        assert remaining == 3
...
# ============================================================================
# WebSocket router import test
# ============================================================================

class TestWebSocketRouter:

    def test_ws_router_imports(self):
        from api.routers.ws_router import router
        # Just verify it imports without error
        assert router is not None

    def test_collect_metrics(self):
        from api.routers.ws_router import _collect_metrics
        metrics = _collect_metrics()
        assert "spill" in metrics
        assert "active_executors" in metrics
        assert "bulletin_counts" in metrics
