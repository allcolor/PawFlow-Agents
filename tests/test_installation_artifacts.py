...
def test_install_scripts_mount_persistent_dirs_and_docker_socket():
    ...
    assert "published_at" in install_src
    assert "created_at" in install_src
    assert "version_key" not in install_src
    assert "sort -k1,1V -k2,2n -k3,3n" not in install_src
    assert "sort -V | tail -n 1" not in install_src
    ...
    assert "Get-VersionSortKey" not in install_ps1_src
    assert "published_at" in install_ps1_src
    assert "created_at" in install_ps1_src
    assert "Sort-Object { if ($_.published_at)" in install_ps1_src
