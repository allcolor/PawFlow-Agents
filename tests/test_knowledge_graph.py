...
- get_hyperedges (3+ objects same subject+predicate)
- surprises (cross-domain scoring)
...
class TestHyperedges(unittest.TestCase):
    ...
    def test_three_plus_objects(self):
        self.kg.add_triple("Alice", "likes", "Python")
        self.kg.add_triple("Alice", "likes", "Rust")
        self.kg.add_triple("Alice", "likes", "Go")
        hyper = self.kg.get_hyperedges()
        assert len(hyper) == 1
        assert hyper[0]["subject"] == "Alice"
        assert hyper[0]["predicate"] == "likes"
        assert hyper[0]["count"] == 3
        assert set(hyper[0]["objects"]) == {"Python", "Rust", "Go"}

    def test_fewer_than_three_excluded(self):
        self.kg.add_triple("A", "r", "X")
        self.kg.add_triple("A", "r", "Y")
        hyper = self.kg.get_hyperedges()
        assert hyper == []
    ...
    def test_expired_excluded(self):
        self.kg.add_triple("A", "r", "X")
        self.kg.add_triple("A", "r", "Y")
        self.kg.add_triple("A", "r", "Z")
        self.kg.invalidate("A", "r", "Z")
        hyper = self.kg.get_hyperedges()
        assert hyper == []


class TestSurprises(unittest.TestCase):
    ...
    def test_cross_domain_scoring(self):
        # Give entities different types
        self.kg.add_triple("Alice", "uses", "Python")
        self.kg._entities["Alice"]["type"] = "person"
        self.kg._entities["Python"]["type"] = "language"
        surprises = self.kg.surprises()
        assert len(surprises) >= 1
        assert surprises[0]["score"] > 0

    def test_ambiguous_confidence_scores_higher(self):
        self.kg.add_triple("A", "r", "B", confidence="AMBIGUOUS")
        surprises = self.kg.surprises()
        assert len(surprises) >= 1
        # AMBIGUOUS gets +3
        assert any(s["score"] >= 3 for s in surprises)
...
class TestDetectCommunities(unittest.TestCase):
    ...
    def test_two_clusters(self):
        ...
        self.kg.add_triple("A", "r", "B")
        self.kg.add_triple("B", "r", "C")
        self.kg.add_triple("A", "r", "C")
        # Cluster 2: X-Y-Z densely connected
        self.kg.add_triple("X", "r", "Y")
        self.kg.add_triple("Y", "r", "Z")
        self.kg.add_triple("X", "r", "Z")
    ...
    def test_single_triple(self):
        ...
        # Both A and B should be in some community
        all_members = []
