...
def test_mcp_bridge_connects_tool_relay_during_startup():
    src = Path("tools/mcp_bridge.py").read_text(encoding="utf-8")
    boot_block = src[
        src.index("def main"):
        src.index("# MCP tools: get_tool_schema + use_tool")]
    assert "# Connect to PawFlow tool relay" in boot_block
    assert "client = ToolRelayClient(" in boot_block
    assert "client.connect()" in boot_block
    assert "Failed to connect to tool relay" in boot_block
...
def test_codex_mcp_config_timeout_is_not_used_for_tool_relay_connect():
    ...
    assert "TOOL_RELAY_CONNECT_TIMEOUT_SECONDS" not in bridge_src
    assert "timeout=TOOL_RELAY_CONNECT_TIMEOUT_SECONDS" not in bridge_src
...
def test_codex_app_mcp_json_string_image_result_never_serializes_base64():
    import json
    from core.llm_providers.codex_app_server import LLMCodexAppServerMixin

    item = {
        "arguments": {"tool_name": "see", "arguments": {"path": "fs://filestore/fid/screen.png"}},
    ...
    assert text == "Image: screenshot.png\nImage reference: fs://filestore/fid/screen.png"
    assert "AAAA" not in text
    assert "data" not in text


def test_codex_app_dynamic_tool_image_result_never_serializes_base64():
    from core.llm_providers.codex_app_server import LLMCodexAppServerMixin
    ...
    item = {
        ...
        "status": "completed",
        "success": True,
        "arguments": {"path": "fs://filestore/fid/screen.png"},
        "contentItems": [
            {"type": "text", "text": "Image: screenshot.png"},
            {"type": "image", "mimeType": "image/png", "data": "A" * 100_000},
        ],
    ...
    text = LLMCodexAppServerMixin._codex_app_native_tool_result(item)

    assert text == "Image: screenshot.png\nImage reference: fs://filestore/fid/screen.png"
    assert "AAAA" not in text
    assert "contentItems" not in text
