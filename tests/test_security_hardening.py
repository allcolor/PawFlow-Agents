...
class TestCORSConfiguration(unittest.TestCase):
    ...
    def test_default_origins(self):
        ...
        env = os.environ.copy()
        env.pop("PYFI2_CORS_ORIGINS", None)
        with patch.dict(os.environ, env, clear=True):
            raw = os.environ.get("PYFI2_CORS_ORIGINS",
    ...
    def test_custom_origins_from_env(self):
        with patch.dict(os.environ, {"PYFI2_CORS_ORIGINS": "https://app.example.com"}):
            raw = os.environ.get("PYFI2_CORS_ORIGINS",
                                 "http://localhost:8501,http://localhost:8000")
            origins = [o.strip() for o in raw.split(",") if o.strip()]
    ...
    def test_wildcard_if_explicit(self):
        with patch.dict(os.environ, {"PYFI2_CORS_ORIGINS": "*"}):
            raw = os.environ.get("PYFI2_CORS_ORIGINS",
                                 "http://localhost:8501,http://localhost:8000")
            origins = [o.strip() for o in raw.split(",") if o.strip()]
...
class TestInputValidation(unittest.TestCase):
    ...
    def test_body_size_default(self):
        ...
        default = int(os.environ.get("PYFI2_MAX_BODY_SIZE", str(10 * 1024 * 1024)))
        self.assertEqual(default, 10 * 1024 * 1024)

    def test_body_size_env_override(self):
        with patch.dict(os.environ, {"PYFI2_MAX_BODY_SIZE": "1024"}):
            val = int(os.environ.get("PYFI2_MAX_BODY_SIZE", str(10 * 1024 * 1024)))
